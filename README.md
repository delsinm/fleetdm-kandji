...
| PCI DSS v4.0          | Req 10.7.2 Critical Control Failures      | Failures of critical security controls detected and reported promptly |
| PCI DSS v4.0          | Req 11.6.1 Change Detection              | Change and tamper detection mechanisms in place and active          |
| GDPR                  | Art. 32(1)(d) Regular Testing            | Regular testing and evaluation of technical security measures       |

Kandji satisfies the enforcement requirement. This system satisfies the independent verification requirement. Having both is a more defensible position with a 3PAO, HIPAA auditor, SOC 2 assessor, or PCI QSA than relying on MDM enforcement alone.
...
The `fleet/policies/` folder contains 36 FleetDM policy definitions. Each policy is a simple check that runs on every managed device and returns pass or fail.

Policies are organised into categories:

| Category                   | Policies                                                                                                      |
| -------------------------- | ------------------------------------------------------------------------------------------------------------- |
| Malware & Threat Detection | EDR software running, EDR version current, EDR kernel extension loaded (choose one EDR from CrowdStrike, Cisco Secure Endpoint, or SentinelOne) |
| Encryption                 | Disk encryption, FileVault key escrowed                                                                       |
| Network & Boundary         | Firewall, SSH, Screen sharing, Remote management, AirDrop, Internet sharing, Printer sharing, Content caching, Bluetooth managed, DNS filtering |
| System Integrity           | SIP enabled, Gatekeeper enabled                                                                               |
| Patching                   | Automatic updates, OS up to date                                                                              |
| Access Control & Identity  | Guest account, Local admin accounts, Screen lock, Password after lock, Password complexity, Login window, Okta Verify installed |
| Audit & Logging            | Audit logging enabled, BSM audit flags configured, NTP time synchronisation                                   |
| Backup & Recovery          | Endpoint backup agent running                                                                                 |
| MDM Health                 | MDM enrollment certificate valid                                                                              |
| PCI DSS Specific           | PCI session idle timeout (Req 8.2.8)                                                                          |

Each policy file documents exactly what it checks, why it matters, and how to fix it if a device fails. Compliance mappings are included for FedRAMP, HIPAA, SOC 2, ISO 27001, GDPR, and PCI DSS v4.0.
...
By default JSON logs go to stdout alongside the human-readable output on stderr. To separate them, set the `JSON_LOG_FILE` environment variable to a file path — the script will append JSON lines there while human-readable logs continue to stderr:
...
│
├── fleet/
│   ├── policies/                    ← FleetDM policy YAML definitions (36 files)
...
└── .github/
    └── workflows/
        ├── fleet-sync.yml           ← GitOps helper — applies Fleet policies on merge
        ├── fleet-plan-apply.yml     ← recommended — plan on PR, apply on merge
...
| `GDPR-MATRIX.md`      | Policy → GDPR Article mapping                   |
| `PCI-DSS-MATRIX.md`   | Policy → PCI DSS v4.0 requirement mapping       |
...
| `SLACK_CHANNEL_ID` | Right-click your Slack channel → Copy channel ID             |
...
This ensures compliance drift is detected within one hour regardless of when it occurs, which satisfies the "continuous monitoring" requirement across FedRAMP, HIPAA, SOC 2, ISO 27001, GDPR, and PCI DSS v4.0. Clean runs produce no Slack message, so there is no operational noise from off-hours execution.
...
1. **Check Fleet policies** — confirm all 36 policies appear in Fleet under Policies and are evaluating (showing pass/fail counts per device).
...
### Example: Updating the EDR version threshold

EDR version policies check against a minimum agent version. Update quarterly or when a new minimum is released:
...
Commit the change and the Fleet sync workflow will apply it on merge. See the individual policy files for review cadence guidance — SentinelOne releases updates more frequently than CrowdStrike or Cisco and should be reviewed monthly rather than quarterly.

---

## Compliance documentation

Full framework mapping is available in the matrix files in the root directory of this repo. Additionally, each policy file also contains inline compliance rationale in its header comments.
